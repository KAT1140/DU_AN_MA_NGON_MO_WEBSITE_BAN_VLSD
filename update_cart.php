...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    // Kiểm tra item thuộc về cart của session hiện tại
    $stmt = $conn->prepare("SELECT ci.id FROM cart_items ci
                           JOIN cart c ON c.id = ci.cart_id
                           WHERE ci.id = ? AND c.session_id = ?");
    if (!$stmt) {
        $response['message'] = 'Lỗi prepare: ' . $conn->error;
        echo json_encode($response);
        exit;
    }
    
    $stmt->bind_param('is', $cart_item_id, $cart_session);
    $stmt->execute();
    ...
    if (!$update_stmt) {
        $response['message'] = 'Lỗi prepare update: ' . $conn->error;
        echo json_encode($response);
        exit;
    }
